...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  // Sort by date
  if (siteConfig('POSTS_SORT_BY', null, NOTION_CONFIG) === 'last') {
    allPages.sort((a, b) => {
      return Date.parse(b?.lastEditedDate) - Date.parse(a?.lastEditedDate)
    })
  }
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION || BLOG.NOTION_PAGE_ID
